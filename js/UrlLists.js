$(function(){    
    ...
    $("#addForm").on("submit", function(event){ 
        ...
        if ( reg.test(URL) && lastUrl !== URL )    {
         ...
         addUrl(URL);    // вызов меню создания ссылки           
         }  else {
         input.tooltip({
            trigger: 'manual', // только по команде
            placement: 'bottom',
            title: 'please set the url here'
         }).tooltip('show');
          setTimeout(function(){ return input.tooltip('destroy')}, 1000);
      }      
    });
    ...
    function addUrl(URL){
     ...
     // и вставить  в соответствующие поля в value
      ...
      // $('div#addUrlField a#saveUrl').on('click', function(){
        ...
        $('div#urlAdd :input').removeAttr('disabled', 'disabled');
        $("div#urlKit :button").removeClass('disabled');
